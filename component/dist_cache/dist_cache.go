...
func (dc *DistCache) CopyToFile(options internal.CopyToFileOptions) error {
	...
	if err == dcache.ErrNotFoundGotLock {
		...
		popCtx, popCancel := context.WithTimeout(context.Background(), 60*time.Second)
		dc.flushMu.Lock()
		dc.flushCancel[options.Name] = popCancel
		dc.flushMu.Unlock()
		go dc.populateCache(popCtx, options.Name, options.File.Name(), dc.getVersion(options.Name))
		return nil
	}
